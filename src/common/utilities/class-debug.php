	...
	class Debug {
		...
		function output_to_log( $log = '', string $email = '' ): void {
			...
			if ( ! empty( $email ) ) {
				...
				if ( $mail_sent ) {
					$message = esc_html_x( 'Email sent.', 'Successfully emailed from output_to_log()', 'joeee-booking' ) . ' ' . $message;
				} else {
					$message = esc_html_x( 'Email attempted but failed.', 'Unsuccessfully emailed from output_to_log()', 'joeee-booking' ) . ' ' . $message;
				}
			}

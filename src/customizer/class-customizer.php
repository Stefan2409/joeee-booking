...
namespace Joeee_Booking\Customizer;

use WP_Customize_Manager;
use Joeee_Booking\Common\Settings as Settings;
use Joeee_Booking\Common\Utilities as Utils;
		...
		public function customizer_options( WP_Customize_Manager $wp_customize ): void {
			...
			$wp_customize->add_panel(
				...
				[
					...
					'description' => esc_html__( 'Plugin options and settings', 'joeee-booking' ) . $this->settings->get_link_to_customizer_panel(),
				]
			);
			...
			$wp_customize->add_section(
				...
				[
					'title'       => esc_html__( 'Example Section', 'joeee-booking' ),
					'description' => esc_html__( 'Example Section description.', 'joeee-booking' ),
		...
		private function add_setting_social_networks( WP_Customize_Manager $wp_customize, string $section_slug, string $setting_slug ): void {
			...
			$wp_customize->add_control(
				new Sortable_Checkboxes_Control(
					...
					[
						'label'       => esc_html__( 'Social Network(s)', 'joeee-booking' ),
						'description' => esc_html__( 'Checked ones will output; unchecked ones will not. Drag and drop to set your preferred display order.', 'joeee-booking' ),
		...
		private function add_setting_post_types( WP_Customize_Manager $wp_customize, string $section_slug, string $setting_slug ): void {
			...
			$wp_customize->add_control(
				new Sortable_Checkboxes_Control(
					...
					Plugin_Data::plugin_text_domain_underscores() . '_' . $setting_slug . '_control',
					[
						'label'       => esc_html__( 'Post Type(s)', 'joeee-booking' ),
						'description' => esc_html__( 'Which Post Types should be enabled?', 'joeee-booking' ),
						'section'     => $this->get_section_id( $section_slug ),
						'settings'    => $this->get_setting_id( $setting_slug ),
